...
""" File name:   covid_simulation.py
    Description: This is the main file for COMP2050 Homework 0 Exercise 4.

                 --- You do not need to modify this file ---

                 python covid_simulation.py to get usage information.
"""
...
def test_scenario_file(module):
    """ Test the given covid_scenario module.
    ...
    print("Checking for CovidScenario class...")
    print("-------------------------------------")
    try:
        ds_class = module.CovidScenario
        if type(ds_class) != type:
            print("Failed. CovidScenario is not a class.")

            class tc:
                pass
            if type(ds_class) == type(tc):
                print("You have forgotten to make CovidScenario subclass object.")
            raise TestingError()
        ds = ds_class()
    except AttributeError:
        print("Failed. CovidScenario does not exist.")
        raise TestingError()
    print("Success.")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Horseman No. 2 - a covid simulator')
    parser.add_argument('-t', '--test', default=False, action='store_true',
                        help='Activates TESTING mode (default: deactivated)')
    parser.add_argument('-s', '--scenario', default=None,
                        type=str, help='Path to scenario (.scn) file')
    parser.add_argument('-a', '--agent', default='CovidEradicationAgent',
                        type=str, help='Agent to be used (default: CovidEradicationAgent)')
...
def main():
    ...
    if args.test:
        ...
        try:
            ds = covid_scenario.CovidScenario()
        ...
        try:
            agent_module = importlib.import_module("covid_eradication_agents")
            base_class = agent_module.CovidEradicationAgent
        except ImportError:
            print("Error loading agent from covid_eradication_agents.py")
            print("Make sure the file exists.")
            sys.exit()
        except Exception:
            print("Unexpected error while trying to import covid_eradication_agents.py:")
            print(traceback.format_exc())
        ...
        if agent_name not in agent_module.__dict__:
            print("There is no such agent as",
                  agent_name, "in covid_eradication_agents.py")
            sys.exit()

        agent_class = agent_module.__dict__[agent_name]
        if not issubclass(agent_class, base_class):
            print("Error:", agent_name, "is not an instance of a CovidEradicationAgent.")
            sys.exit()
            ...
            for j in range(0, args.num_sims):
                ...
                for i in range(iterations):
                    ...
                    if total_covid < 0.001:
                        print("You eradicated all covid! Good job.")
                        break
                    if total_covid >= 100000:
                        print("Total covid went over 100000.")
                        print("Covid has taken over Kosciuszko National Park and its surroundings,")
                        print("and has driven native flora and fauna to extinction.")
                        break
                sim_data = {}
